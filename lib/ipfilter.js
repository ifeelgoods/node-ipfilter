 ...
 * @api public
 */
 module.exports = function ipfilter(ipsGetter, opts)
 {
 	// ipsGetter is either a static array of IPs 
 	// or a function : function(req) {return ipsArray;} 
 	var ips;

 	var settings = _.defaults(opts || {}, {
 		mode: 'deny', log: true, errorCode: 401, errorMessage: 'Unauthorized'
 	}),
 	matchIp = function(ip, ips)
 	{
 	...
 	{
		if (typeof(ipsGetter) === 'function') {
			ips = ipsGetter(req) || false; 
		} else {
			ips = ipsGetter || false;
		}
